...
def main():
    ...
    steps=1000
    #load model
    model = load_model("Unet-model_15.h5", custom_objects={'dice_coef_loss': dice_coef_loss, 'dice_coef': dice_coef})
    loss, accuracy = model.evaluate_generator(test_data, steps=steps)
    print('\ntest loss', loss)
    print('dice_coef', accuracy)

    #continue train
    history=model.fit_generator(train_data,steps_per_epoch=steps*8,epochs=20,verbose=2,
                                validation_data=validation_data,validation_steps=steps,initial_epoch=15)
    #save history
    f=h5py.File("Unet-history_20.h5","w")
